...
print('Make sure s1 goes into state 0 after send.')
s1.send('msg2')
socks = dict(poller.poll())
assert not socks.has_key(s1)
...
print('Wait and then see that s2 is in POLLIN.')
time.sleep(0.5)
socks = dict(poller.poll())
...
print('Make sure that s2 is in POLLOUT after recv.')
s2.recv()
socks = dict(poller.poll())
assert socks[s2] == zmq.POLLOUT
...
print('Wait for everything to finish.')
time.sleep(1.0)
