...
class game ():
	...
	def StartNewGame (self,thisLevel,thisGame,thisFruit,player,tileIDName,tileID,tileIDImage,path):
		self.levelNum = 1
		self.score = 0
		self.lives = 3
		
		self.SetMode( 4 )
		thisLevel.LoadLevel( thisGame.GetLevelNum(),thisFruit,player,tileIDName,tileID,tileIDImage,path,thisLevel,thisGame)
...
class level ():
	...
	def IsWall (self, (row, col),thisLevel):
	...
	def DrawMap (self,thisGame,tileID,tileIDImage):
	...
	def LoadLevel (self, levelNum,thisFruit2,player,tileIDName,tileID,tileIDImage,path,thisLevel,thisGame):
		...
		for line in f:
			...
			if (j == "'" or j == ""):
				...
				if firstWord == "lvlwidth":
					...
					thisFruit2.fruitType = int( str_splitBySpace[2] )
		...
		GetCrossRef(tileIDName,tileID,tileIDImage,self)
		...
		for row in range(0, path.size[0], 1):
			for col in range(0, path.size[1], 1):
				if self.IsWall( (row, col), thisLevel):
					path.SetType( (row, col), 1 )
				else:
					path.SetType( (row, col), 0 )

		# do all the level-starting stuff
		self.Restart(thisLevel,thisGame,player)
		
	def Restart (self,thisFruit,thisGame,player):
		'''
		for i in range(0, 4, 1):
...
def CheckInputs(thisGame,js,thisLevel,thisFruit,player,tileIDName,tileID,tileIDImage,path): 
	...
	if thisGame.mode == 1:
		...
		if pygame.key.get_pressed()[ pygame.K_RETURN ] or (js!=None and js.get_button(JS_STARTBUTTON)):
			thisGame.StartNewGame(thisLevel,thisGame,thisFruit,player,tileIDName,tileID,tileIDImage,path)
...
def main2():
	...
	thisGame = game()
	thisLevel = level()
	#thisLevel.LoadLevel( thisGame.GetLevelNum(), thisLevel)
...
def main():
	...
	thisFruit=fruit()
	tileIDName={} #gives tile name (when the iD# is known)
	tileID={} #gives tile ID (when name is known)
	tileIDImage={} #gives tile image(when the ID# is kwonn)

	thisGame=game()
	thisLevel=level()
	thisLevel.LoadLevel(thisGame.GetLevelNum(),thisFruit,player1,tileIDName,tileID,tileIDImage,path,thisLevel,thisGame)
	...
	while(True):
		...
		if thisGame.mode==1:
			...
			# game over
			CheckInputs(thisGame,js,thisLevel,thisFruit,player1,tileIDName,tileID,tileIDImage,path)
		...
		#thisGame.SmartMoveScreen(player1,thisLevel,thisGame)
		
		screen.blit(img_Background, (0, 0))
		
		if not thisGame.mode == 8:
			thisLevel.DrawMap(thisGame,tileID,tileIDImage)
